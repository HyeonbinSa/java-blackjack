...
### 2차 피드백 
- 현재 구조상의 문제가 무엇인지 이해하고 이를 해결할 수 있는 방법을 찾는다는 생각을 하면 좋을 것 같다.
  - 디자인 패턴은 흔하게 사용되는 몇가지 해결법들을 정리한 것으로 보면된다.
  - 현재 구현된 상태를 분리하는 것이 오히려 관리를 어렵게 만든다면 기존의 방식이 최선일 수 있다.
- `of`, `to` 와 같은 전치사로 끝나는 메서드는 인자가 <u>하나</u>인 경우가 많으며 자연스럽다.
  - ex) `writeResultTo(Writer writer)`,`calculateProfitRateOf(Player player)`
  - 하지만 네이밍의 방식 중 하나이니 인자가 하나인 모든 경우에 적합하다는 뜻은 아님.
- `gambler`의 상태를 가져와서 `gambler.getMoney()`를 전달해 수익을 계산.
  - 더 단순한 형태는 없을지에 대한 고민
